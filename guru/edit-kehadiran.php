...
$sql = "SELECT k.*, s.nis, s.nama_siswa, ks.nama_kelas, 
...
$stmt->execute([$id_kehadiran]);
$kehadiran = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$kehadiran) {
    $_SESSION['error'] = "Data kehadiran tidak ditemukan!";
    header("Location: kehadiran.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Cek apakah kehadiran untuk siswa, jadwal, dan tanggal ini sudah ada (kecuali data yang sedang diedit)
        $checkSql = "SELECT id_kehadiran FROM kehadiran WHERE id_siswa = ? AND id_jadwal = ? AND tanggal = ? AND id_kehadiran != ?";
        $checkStmt = $pdo->prepare($checkSql);
        $checkStmt->execute([$id_siswa, $id_jadwal, $tanggal, $id_kehadiran]);
        ...
        // Update data
        $updateSql = "UPDATE kehadiran SET id_siswa = ?, id_jadwal = ?, tanggal = ?, status = ?, keterangan = ? WHERE id_kehadiran = ?";
        $updateStmt = $pdo->prepare($updateSql);
        $updateStmt->execute([$id_siswa, $id_jadwal, $tanggal, $status, $keterangan, $id_kehadiran]);
...
<head>
    ...
    <title>Edit Kehadiran - Sistem Sekolah</title>
...
<body>
    <div class="flex h-screen">
        <!-- Sidebar (Sesuai dengan kehadiran.php) -->
        <aside class="sidebar flex flex-col p-4">
            ...
            <!-- Navigasi (Sesuai dengan kehadiran.php) -->
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Edit Kehadiran</div>
                        <div class="text-slate-500 text-xs truncate">Form edit data kehadiran siswa</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-clipboard-user text-blue-600"></i> Edit Kehadiran
                        </h1>
                        <p class="text-slate-500 mt-1">Form edit data kehadiran siswa</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-500 mb-6">
                        ...
                        <span class="text-slate-700 font-medium">Edit Kehadiran</span>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                            <div>
                                <span class="text-blue-200">Siswa:</span>
                                <div class="font-semibold"><?= htmlspecialchars($kehadiran['nis'] . ' - ' . $kehadiran['nama_siswa']) ?></div>
                            </div>
                            <div>
                                <span class="text-blue-200">Kelas:</span>
                                <div class="font-semibold"><?= htmlspecialchars($kehadiran['nama_kelas']) ?></div>
                            </div>
                            <div>
                                <span class="text-blue-200">Mapel:</span>
                                <div class="font-semibold"><?= htmlspecialchars($kehadiran['nama_mapel']) ?></div>
                            </div>
                            <div>
                                <span class="text-blue-200">Status:</span>
                                <div class="font-semibold"><?= htmlspecialchars($kehadiran['status']) ?></div>
                            </div>
                        </div>
                    </div>
                            ...
                            <form method="POST" id="kehadiranForm">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <select name="id_siswa" id="id_siswa" class="form-select" required>
                                            ...
                                            <?php foreach ($siswa as $s): ?>
                                                <option value="<?= $s['id_siswa'] ?>"
                                                    <?= (isset($_POST['id_siswa']) ? $_POST['id_siswa'] : $kehadiran['id_siswa']) == $s['id_siswa'] ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($s['nis'] . ' - ' . $s['nama_siswa']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <div class="form-help">Pilih siswa yang akan diedit kehadirannya</div>
                                    </div>
                                        ...
                                        <select name="id_jadwal" id="id_jadwal" class="form-select" required>
                                            ...
                                            <?php foreach ($jadwal as $j): ?>
                                                <option value="<?= $j['id_jadwal'] ?>"
                                                    ...
                                                    <?= (isset($_POST['id_jadwal']) ? $_POST['id_jadwal'] : $kehadiran['id_jadwal']) == $j['id_jadwal'] ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($j['nama_mapel'] . ' - ' . $j['nama_petugas']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <div class="form-help">Pilih jadwal pelajaran</div>
                                        <div id="jadwalInfo" class="jadwal-info">
                                            <div class="text-sm">
                                                <div><strong>Mata Pelajaran:</strong> <span id="infoMapel"><?= htmlspecialchars($kehadiran['nama_mapel']) ?></span></div>
                                                <div><strong>Guru:</strong> <span id="infoGuru"><?= htmlspecialchars($kehadiran['nama_petugas']) ?></span></div>
                                                <div><strong>Hari:</strong> <span id="infoHari"><?= htmlspecialchars($kehadiran['hari']) ?></span></div>
                                                <div><strong>Jam:</strong> <span id="infoJam"><?= htmlspecialchars($kehadiran['jam_mulai'] . ' - ' . $kehadiran['jam_selesai']) ?></span></div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="tanggal" id="tanggal"
                                            value="<?= htmlspecialchars(isset($_POST['tanggal']) ? $_POST['tanggal'] : $kehadiran['tanggal']) ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="flex items-center">
                                            <select name="status" id="status" class="form-select" required>
                                                ...
                                                <option value="Hadir" <?= (isset($_POST['status']) ? $_POST['status'] : $kehadiran['status']) == 'Hadir' ? 'selected' : '' ?>>Hadir</option>
                                                <option value="Izin" <?= (isset($_POST['status']) ? $_POST['status'] : $kehadiran['status']) == 'Izin' ? 'selected' : '' ?>>Izin</option>
                                                <option value="Sakit" <?= (isset($_POST['status']) ? $_POST['status'] : $kehadiran['status']) == 'Sakit' ? 'selected' : '' ?>>Sakit</option>
                                                <option value="Alpha" <?= (isset($_POST['status']) ? $_POST['status'] : $kehadiran['status']) == 'Alpha' ? 'selected' : '' ?>>Alpha</option>
                                            </select>
                                            <span id="statusPreview" class="status-preview">
                                                <?= htmlspecialchars($kehadiran['status']) ?>
                                            </span>
                                        </div>
                                    ...
                                    <div class="form-group md:col-span-2">
                                        ...
                                        <textarea name="keterangan" id="keterangan"
                                            ...
                                            rows="3"><?= htmlspecialchars(isset($_POST['keterangan']) ? $_POST['keterangan'] : $kehadiran['keterangan']) ?></textarea>
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 justify-end mt-8 pt-6 border-t border-slate-200">
                                    ...
                                    <button type="submit" class="btn btn-success">
                                        <i class="fa-solid fa-floppy-disk mr-2"></i> Update Kehadiran
                                    </button>
                                </div>
                            </form>
                        </div>
