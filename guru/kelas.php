...
$stmt = $pdo->prepare("SELECT id_kelas, nama_kelas, tingkat FROM kelas ORDER BY 
                      CASE 
                          WHEN tingkat = 'X' THEN 1
                          WHEN tingkat = 'XI' THEN 2
                          WHEN tingkat = 'XII' THEN 3
                          ELSE 4
                      END, nama_kelas ASC");
$stmt->execute();
...
foreach ($kelas as $k) {
    $tingkat = $k['tingkat'];
    if (isset($tingkatCount[$tingkat])) {
        $tingkatCount[$tingkat]++;
    }
}
...
<head>
    ...
    <title>Data Kelas - Sistem Sekolah</title>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Data Kelas</div>
                        <div class="text-slate-500 text-xs truncate">Manajemen kelas sekolah</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-door-open text-blue-600"></i> Data Kelas
                        </h1>
                        <p class="text-slate-500 mt-1">Manajemen kelas sekolah berdasarkan tingkat</p>
                    </div>
                ...
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6 auto-rows-fr">
                        <div class="card stat-card">
                            ...
                            <p class="stat-value text-yellow-500"><?= $totalKelas; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-red-500">
                                <i class="fa-solid fa-1"></i>
                            </div>
                            <h3 class="stat-label">Kelas X</h3>
                            <p class="stat-value text-red-500"><?= $tingkatCount['X']; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-green-600">
                                <i class="fa-solid fa-2"></i>
                            </div>
                            <h3 class="stat-label">Kelas XI</h3>
                            <p class="stat-value text-green-600"><?= $tingkatCount['XI']; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-blue-600">
                                <i class="fa-solid fa-3"></i>
                            </div>
                            <h3 class="stat-label">Kelas XII</h3>
                            <p class="stat-value text-blue-600"><?= $tingkatCount['XII']; ?></p>
                        </div>
                    </div>
                    ...
                    <div class="card p-4 mb-6">
                        <?php if (isset($_SESSION['flash']) && $_SESSION['flash'] === "success"): ?>
                            <div class="success-message mb-4 flex items-center justify-between">
                                <span><i class="fa-solid fa-circle-check mr-2"></i> Kelas baru berhasil ditambahkan!</span>
                        ...
                        <?php if (isset($_SESSION['flash']) && $_SESSION['flash'] === "updated"): ?>
                            <div class="info-message mb-4 flex items-center justify-between">
                                <span><i class="fa-solid fa-pen-to-square mr-2"></i> Data kelas berhasil diperbarui!</span>
                        ...
                        <?php if (isset($_SESSION['flash']) && $_SESSION['flash'] === "deleted"): ?>
                            <div class="warning-message mb-4 flex items-center justify-between">
                                <span><i class="fa-solid fa-trash mr-2"></i> Kelas berhasil dihapus!</span>
                        ...
                        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                            <a href="tambah-kelas.php" class="btn btn-success">
                                <i class="fa-solid fa-plus mr-2"></i> Tambah Kelas
                            </a>

                            <div class="w-full lg:w-64">
                                <input type="text" id="searchInput" placeholder="Cari Nama Kelas / Tingkat..."
                                    class="search-input">
                            </div>
                        </div>

                        <!-- Tabel Kelas Berdasarkan Tingkat -->
                        <?php
                        ...
                        foreach ($kelas as $k) {
                            if (isset($kelasByTingkat[$k['tingkat']])) {
                        ...
                        <?php foreach (['X', 'XI', 'XII'] as $tingkat): ?>
                            <?php if (!empty($kelasByTingkat[$tingkat])): ?>
                                <div class="tingkat-section mb-6">
                                    <div class="tingkat-header <?= strtolower($tingkat) ?> mb-0">
                                        <h3 class="text-lg font-bold flex items-center gap-2">
                                            <i class="fa-solid fa-layer-group"></i>
                                            <?= $tingkatLabels[$tingkat] ?>
                                            <span class="bg-white/20 px-2 py-1 rounded text-sm">
                                                <?= count($kelasByTingkat[$tingkat]) ?> Kelas
                                            </span>
                                        </h3>
                                    </div>
                                    <div class="table-scroll">
                                        <table class="w-full text-left border-collapse">
                                            ...
                                            <tbody>
                                                <?php $no = 1; ?>
                                                <?php foreach ($kelasByTingkat[$tingkat] as $k): ?>
                                                    <tr class="border-t hover:bg-slate-50">
                                                        <td class="py-3 px-4"><?= $no++; ?></td>
                                                        <td class="py-3 px-4 nama"><?= htmlspecialchars($k['nama_kelas']); ?></td>
                                                        <td class="py-3 px-4 tingkat">
                                                        ...
                                                        <td class="py-3 px-4">
                                                            <div class="flex flex-wrap gap-2">
                                                                <a href="edit-kelas.php?id=<?= $k['id_kelas']; ?>"
                                                                    class="btn btn-warning px-3 py-1 text-sm">
                                                                    <i class="fa-solid fa-pen mr-1"></i> Edit
                                                                </a>
                                                                <a href="hapus-kelas.php?id=<?= $k['id_kelas']; ?>"
                                                                    onclick="return confirm('Hapus kelas <?= htmlspecialchars($k['nama_kelas']) ?>?');"
                        ...
                        <?php if (empty($kelas)): ?>
                            <div class="empty-state">
                                ...
                                <p class="text-sm mt-2">Klik tombol "Tambah Kelas" untuk menambahkan data</p>
                            </div>
                        <?php endif; ?>
                    </div>

                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Kelas Management
                    </footer>
                </main>
            </div>
        </div>
    ...
    <script>
        ...
        searchInput.addEventListener("keyup", () => {
            ...
            rows.forEach(row => {
                ...
                let tingkat = row.querySelector(".tingkat")?.innerText.toLowerCase() || '';

                if (nama.includes(filter) || tingkat.includes(filter)) {
                    row.style.display = "";
                } else {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            // Ctrl+Shift+K untuk tambah kelas
