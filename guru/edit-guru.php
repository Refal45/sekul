...
try {
    $stmt = $pdo->prepare("SELECT id_petugas, username, nama_petugas, nip, no_hp FROM petugas WHERE id_petugas = ? AND role='guru'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Tambahkan baris ini untuk mengkonversi string kosong menjadi NULL
    $nip = empty($nip) ? null : $nip;

    // Validasi
    if (empty($username_input) || empty($nama_petugas)) {
        ...
        try {
            // Cek apakah username sudah digunakan oleh guru lain
            $stmt = $pdo->prepare("SELECT id_petugas FROM petugas WHERE username = ? AND id_petugas != ?");
            $stmt->execute([$username_input, $id_petugas]);
            ...
            if ($stmt->fetch()) {
                ...
                if (!empty($password)) {
                    ...
                    $stmt = $pdo->prepare("UPDATE petugas SET username = ?, password = ?, nama_petugas = ?, nip = ?, no_hp = ? WHERE id_petugas = ?");
                    $stmt->execute([$username_input, $hashed_password, $nama_petugas, $nip, $no_hp, $id_petugas]);
                } else {
                    // Update tanpa mengubah password
                    $stmt = $pdo->prepare("UPDATE petugas SET username = ?, nama_petugas = ?, nip = ?, no_hp = ? WHERE id_petugas = ?");
                    $stmt->execute([$username_input, $nama_petugas, $nip, $no_hp, $id_petugas]);
                }
        ...
        <aside class="sidebar flex flex-col p-4">
            ...
            <nav class="flex-1 space-y-1">
                ...
                <div class="sidebar-section">
                    ...
                    <a href="guru.php" class="bg-blue-600 text-white rounded-md active">
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Edit Guru</div>
                        <div class="text-slate-500 text-xs truncate">Edit data guru</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-user-edit text-yellow-600"></i> Edit Data Guru
                        </h1>
                        <p class="text-slate-500 mt-1">Perbarui informasi akun guru</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-600 mb-6">
                        <a href="guru.php" class="hover:text-blue-600 transition-colors">
                            <i class="fa-solid fa-user-tie mr-1"></i>Data Guru
                        </a>
                        <i class="fa-solid fa-chevron-right text-xs"></i>
                        <span class="text-blue-600 font-medium">Edit Guru</span>
                    </div>
                    ...
                    <div class="info-box">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="font-bold text-blue-900">Sedang mengedit data guru</h3>
                                <p class="text-blue-700 text-sm mt-1">
                                    Username: <strong><?= htmlspecialchars($guru['username']); ?></strong> |
                                    Nama: <strong><?= htmlspecialchars($guru['nama_petugas']); ?></strong>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card p-6">
                        <form method="POST" action="" class="max-w-2xl">
                            <input type="hidden" name="id_petugas" value="<?= $guru['id_petugas']; ?>">
                                ...
                                <div class="space-y-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="username" name="username"
                                            value="<?= htmlspecialchars($_POST['username'] ?? $guru['username']); ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="nama_petugas" name="nama_petugas"
                                            value="<?= htmlspecialchars($_POST['nama_petugas'] ?? $guru['nama_petugas']); ?>"
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 mt-8 pt-6 border-t border-slate-200">
                                ...
                                <a href="guru.php" class="btn btn-secondary flex-1 text-center">
                    ...
                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Guru Management
                    </footer>
                </main>
            </div>
        </div>
