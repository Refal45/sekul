...
$stmt_kelas = $pdo->prepare("SELECT * FROM kelas WHERE id_kelas = ?");
$stmt_kelas->execute([$id_kelas]);
$kelas = $stmt_kelas->fetch(PDO::FETCH_ASSOC);

// Jika kelas tidak ditemukan, redirect
if (!$kelas) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Cek apakah kelas sudah ada (kecuali untuk kelas ini)
        $stmt_check = $pdo->prepare("SELECT id_kelas FROM kelas WHERE nama_kelas = ? AND tingkat = ? AND id_kelas != ?");
        $stmt_check->execute([$nama_kelas, $tingkat, $id_kelas]);
        if ($stmt_check->fetch()) {
        ...
        $stmt->execute([$nama_kelas, $tingkat, $id_kelas]);
...
<head>
    ...
    <title>Edit Kelas - Sistem Sekolah</title>
...
<body>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Edit Kelas</div>
                        <div class="text-slate-500 text-xs truncate">Form edit data kelas</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-door-open text-yellow-600"></i> Edit Kelas
                        </h1>
                        <p class="text-slate-500 mt-1">Form edit data kelas</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="card p-6 mb-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-lg font-semibold text-slate-700">
                                <i class="fa-solid fa-door-open mr-2"></i> Form Edit Kelas
                            </h2>
                        ...
                        <div class="info-box mb-6">
                            <h3 class="font-semibold text-blue-800 mb-2 flex items-center">
                                <i class="fa-solid fa-info-circle mr-2"></i> Informasi Kelas
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                <div><span class="font-medium">ID Kelas:</span> <?= htmlspecialchars($kelas['id_kelas']) ?></div>
                                <div><span class="font-medium">Status:</span>
                                    <span class="badge badge-<?= strtolower($kelas['tingkat']) ?>">
                        ...
                        <form method="POST" class="space-y-6">
                                ...
                                <div class="space-y-4">
                                    <div class="form-group">
                                        ...
                                        <select id="tingkat" name="tingkat" required class="form-select">
                                            ...
                                            <option value="X" <?= ($kelas['tingkat'] === 'X') ? 'selected' : '' ?>>Kelas X (Sepuluh)</option>
                                            <option value="XI" <?= ($kelas['tingkat'] === 'XI') ? 'selected' : '' ?>>Kelas XI (Sebelas)</option>
                                            <option value="XII" <?= ($kelas['tingkat'] === 'XII') ? 'selected' : '' ?>>Kelas XII (Dua Belas)</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="tingkat-preview">
                                    <div class="preview-item preview-x <?= ($kelas['tingkat'] === 'X') ? 'active' : '' ?>" data-value="X" onclick="selectTingkat('X')">
                                    ...
                                    <div class="preview-item preview-xi <?= ($kelas['tingkat'] === 'XI') ? 'active' : '' ?>" data-value="XI" onclick="selectTingkat('XI')">
                                        <div class="preview-icon">
                                    ...
                                    <div class="preview-item preview-xii <?= ($kelas['tingkat'] === 'XII') ? 'active' : '' ?>" data-value="XII" onclick="selectTingkat('XII')">
                                        <div class="preview-icon">
                            ...
                            <div class="bg-slate-50 border border-slate-200 rounded-lg p-4">
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div id="previewBadge" class="badge badge-<?= strtolower($kelas['tingkat']) ?>">
                                        <?= htmlspecialchars($kelas['tingkat']) ?>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-200">
                                <button type="submit" class="btn btn-success flex-1">
                                    <i class="fa-solid fa-save mr-2"></i> Update Kelas
                                </button>
                    ...
                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Edit Kelas
                    </footer>
                </main>
            </div>
        </div>
    ...
    <script>
        ...
        function resetForm() {
            ...
            document.getElementById('nama_kelas').value = '<?= htmlspecialchars($kelas['nama_kelas']) ?>';
            document.getElementById('tingkat').value = '<?= htmlspecialchars($kelas['tingkat']) ?>';

            // Update preview
            document.querySelectorAll('.preview-item').forEach(item => {
                item.classList.remove('active');
            });
            document.querySelector(`.preview-item[data-value="<?= $kelas['tingkat'] ?>"]`).classList.add('active');
            updatePreviewBadge();
        }
