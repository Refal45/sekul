...
$sql = "SELECT n.*, s.nis, s.nama_siswa, k.nama_kelas, mp.nama_mapel 
        FROM nilai n 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Validasi
        if (empty($id_siswa) || empty($id_mapel) || empty($semester) || empty($nilai_value)) {
            throw new Exception("Semua field harus diisi!");
        }

        if (!is_numeric($nilai_value) || $nilai_value < 0 || $nilai_value > 100) {
            throw new Exception("Nilai harus antara 0 - 100!");
        }

        // Cek apakah nilai untuk siswa, mapel, dan semester ini sudah ada (kecuali data yang sedang diedit)
        $checkSql = "SELECT id_nilai FROM nilai WHERE id_siswa = ? AND id_mapel = ? AND semester = ? AND id_nilai != ?";
        $checkStmt = $pdo->prepare($checkSql);
        $checkStmt->execute([$id_siswa, $id_mapel, $semester, $id_nilai]);
        ...
        // Update data
        $updateSql = "UPDATE nilai SET id_siswa = ?, id_mapel = ?, semester = ?, nilai = ? WHERE id_nilai = ?";
        $updateStmt = $pdo->prepare($updateSql);
        $updateStmt->execute([$id_siswa, $id_mapel, $semester, $nilai_value, $id_nilai]);

        $_SESSION['flash'] = "updated";
        header("Location: nilai.php?success=update");
        exit;
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
<head>
    ...
    <title>Edit Nilai - Sistem Sekolah</title>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Edit Nilai</div>
                        <div class="text-slate-500 text-xs truncate">Form edit data nilai siswa</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-file-signature text-blue-600"></i> Edit Nilai
                        </h1>
                        <p class="text-slate-500 mt-1">Form edit data nilai siswa</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-500 mb-6">
                        ...
                        <span class="text-slate-700 font-medium">Edit Nilai</span>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                            <div>
                                ...
                                <div class="font-semibold"><?= htmlspecialchars($nilai['nis'] . ' - ' . $nilai['nama_siswa']) ?></div>
                            </div>
                            <div>
                                <span class="text-blue-200">Kelas:</span>
                                <div class="font-semibold"><?= htmlspecialchars($nilai['nama_kelas']) ?></div>
                            </div>
                            <div>
                                <span class="text-blue-200">Mapel:</span>
                                <div class="font-semibold"><?= htmlspecialchars($nilai['nama_mapel']) ?></div>
                            </div>
                            <div>
                                <span class="text-blue-200">Nilai:</span>
                                <div class="font-semibold"><?= htmlspecialchars($nilai['nilai']) ?></div>
                            </div>
                        </div>
                    </div>
                            ...
                            <form method="POST" id="nilaiForm">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <select name="id_siswa" id="id_siswa" class="form-select" required>
                                            ...
                                            <?php foreach ($siswa as $s): ?>
                                                <option value="<?= $s['id_siswa'] ?>"
                                                    <?= (isset($_POST['id_siswa']) ? $_POST['id_siswa'] : $nilai['id_siswa']) == $s['id_siswa'] ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($s['nis'] . ' - ' . $s['nama_siswa']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <div class="form-help">Pilih siswa yang akan diedit nilainya</div>
                                    </div>
                                        ...
                                        <select name="id_mapel" id="id_mapel" class="form-select" required>
                                            ...
                                            <?php foreach ($mapel as $m): ?>
                                                <option value="<?= $m['id_mapel'] ?>"
                                                    <?= (isset($_POST['id_mapel']) ? $_POST['id_mapel'] : $nilai['id_mapel']) == $m['id_mapel'] ? 'selected' : '' ?>>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="semester" id="semester" class="form-select" required>
                                            ...
                                            <option value="1" <?= (isset($_POST['semester']) ? $_POST['semester'] : $nilai['semester']) == '1' ? 'selected' : '' ?>>Semester 1</option>
                                            <option value="2" <?= (isset($_POST['semester']) ? $_POST['semester'] : $nilai['semester']) == '2' ? 'selected' : '' ?>>Semester 2</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="flex items-center">
                                            <input type="number" name="nilai" id="nilai"
                                                ...
                                                value="<?= htmlspecialchars(isset($_POST['nilai']) ? $_POST['nilai'] : $nilai['nilai']) ?>"
                                                class="form-input" placeholder="0 - 100" required>
                                            <span id="nilaiPreview" class="nilai-preview">
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 justify-end mt-8 pt-6 border-t border-slate-200">
                                    ...
                                    <button type="submit" class="btn btn-success">
                                        <i class="fa-solid fa-floppy-disk mr-2"></i> Update Nilai
                                    </button>
                                </div>
                            </form>
                        </div>
