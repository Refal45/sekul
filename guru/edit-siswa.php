...
try {
    ...
    $kelasStmt = $pdo->query("SELECT * FROM kelas ORDER BY tingkat, nama_kelas");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $username_inp = trim($_POST['username'] ?? '');
    ...
    // Validasi
    if (empty($nis) || empty($nama_siswa) || empty($username_inp) || empty($id_kelas) || empty($jenis_kelamin)) {
        ...
        try {
            // Cek apakah username sudah digunakan oleh siswa lain
            $stmt = $pdo->prepare("SELECT id_siswa FROM siswa WHERE username = ? AND id_siswa != ?");
            $stmt->execute([$username_inp, $id]);
            ...
            if ($stmt->fetch()) {
                ...
                if (!empty($password)) {
                    ...
                    $sql = "UPDATE siswa SET nis=?, nama_siswa=?, username=?, password=?, id_kelas=?, no_hp=?, jenis_kelamin=? WHERE id_siswa=?";
                    $params = [$nis, $nama_siswa, $username_inp, $hashed, $id_kelas, $no_hp, $jenis_kelamin, $id];
                } else {
                    // Update tanpa mengubah password
                    $sql = "UPDATE siswa SET nis=?, nama_siswa=?, username=?, id_kelas=?, no_hp=?, jenis_kelamin=? WHERE id_siswa=?";
                    $params = [$nis, $nama_siswa, $username_inp, $id_kelas, $no_hp, $jenis_kelamin, $id];
                }
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-slate-500 text-xs truncate">Edit data siswa</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-user-edit text-indigo-600"></i> Edit Data Siswa
                        </h1>
                        <p class="text-slate-500 mt-1">Perbarui informasi akun siswa</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-600 mb-6">
                        <a href="siswa.php" class="hover:text-blue-600 transition-colors">
                            <i class="fa-solid fa-user-graduate mr-1"></i>Data Siswa
                        </a>
                        <i class="fa-solid fa-chevron-right text-xs"></i>
                        <span class="text-blue-600 font-medium">Edit Siswa</span>
                    </div>
                    ...
                    <div class="info-box">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="font-bold text-blue-900">Sedang mengedit data siswa</h3>
                                <p class="text-blue-700 text-sm mt-1">
                                    Username: <strong><?= htmlspecialchars($siswa['username']); ?></strong> |
                                    Nama: <strong><?= htmlspecialchars($siswa['nama_siswa']); ?></strong>
                                </p>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="space-y-4">
                                    <div class="form-group">
                                        <label for="nis" class="form-label">
                                            <i class="fa-solid fa-id-card mr-2"></i>NIS *
                                        </label>
                                        <input type="text" id="nis" name="nis"
                                            value="<?= htmlspecialchars($_POST['nis'] ?? $siswa['nis']); ?>"
                                            class="form-input <?= $error && empty($_POST['nis']) ? 'error' : '' ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="nama_siswa" name="nama_siswa"
                                            value="<?= htmlspecialchars($_POST['nama_siswa'] ?? $siswa['nama_siswa']); ?>"
                                            class="form-input <?= $error && empty($_POST['nama_siswa']) ? 'error' : '' ?>"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="username" name="username"
                                            value="<?= htmlspecialchars($_POST['username'] ?? $siswa['username']); ?>"
                                ...
                                <div class="space-y-4">
                                    <div class="form-group">
                                        <label for="password" class="form-label">
                                            <i class="fa-solid fa-lock mr-2"></i>Password Baru
                                        </label>
                                        <input type="password" id="password" name="password"
                                            class="form-input <?= $error && isset($_POST['password']) && strlen($_POST['password']) < 6 ? 'error' : '' ?>"
                                            placeholder="Kosongkan jika tidak ingin mengubah">
                                        <div id="passwordStrength" class="password-strength"></div>
                                        <div class="form-help">Kosongkan jika tidak ingin mengubah password. Minimal 6 karakter.</div>
                                    </div>

                                    <div class="form-group">
                                        <label for="confirm_password" class="form-label">
                                            <i class="fa-solid fa-lock mr-2"></i>Konfirmasi Password Baru
                                        </label>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="id_kelas" name="id_kelas" required
                                            class="form-input <?= $error && empty($_POST['id_kelas']) ? 'error' : '' ?>">
                                            <option value="">-- Pilih Kelas --</option>
                                            <?php foreach ($kelasList as $k): ?>
                                                <option value="<?= $k['id_kelas']; ?>" 
                                                    <?= ($k['id_kelas'] == ($_POST['id_kelas'] ?? $siswa['id_kelas'])) ? 'selected' : ''; ?>>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="jenis_kelamin" name="jenis_kelamin" required
                                        class="form-input <?= $error && empty($_POST['jenis_kelamin']) ? 'error' : '' ?>">
                                        <option value="">-- Pilih Jenis Kelamin --</option>
                                        <option value="L" <?= (($_POST['jenis_kelamin'] ?? $siswa['jenis_kelamin']) === 'L') ? 'selected' : ''; ?>>Laki-laki</option>
                                        <option value="P" <?= (($_POST['jenis_kelamin'] ?? $siswa['jenis_kelamin']) === 'P') ? 'selected' : ''; ?>>Perempuan</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-700">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-600 mt-1">Pastikan data yang diubah sudah benar sebelum menyimpan</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-600 mt-1">Password hanya akan diubah jika field password diisi</p>
                                </div>
                            </div>
                        </div>
                    </div>
