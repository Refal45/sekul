...
// Ambil data nilai join siswa + kelas + mapel
$sql = "SELECT n.id_nilai, s.nis, s.nama_siswa, k.nama_kelas, mp.nama_mapel, n.semester, n.nilai
        FROM nilai n
...
<head>
    ...
    <title>Data Nilai - Sistem Sekolah</title>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Data Nilai</div>
                        <div class="text-slate-500 text-xs truncate">Manajemen data nilai siswa</div>
                    </div>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-file-signature text-blue-600"></i> Data Nilai
                        </h1>
                        <p class="text-slate-500 mt-1">Manajemen data nilai siswa</p>
                    </div>
                ...
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6 auto-rows-fr">
                        <div class="card stat-card">
                            ...
                            <h3 class="stat-label">Total Nilai</h3>
                            <p class="stat-value text-green-600"><?= $totalNilai; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-green-500">
                                <i class="fa-solid fa-star"></i>
                            </div>
                            <h3 class="stat-label">Sangat Baik</h3>
                            <p class="stat-value text-green-500"><?= $nilaiSangatBaik; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-blue-500">
                                <i class="fa-solid fa-thumbs-up"></i>
                            </div>
                            <h3 class="stat-label">Baik</h3>
                            <p class="stat-value text-blue-500"><?= $nilaiBaik; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-yellow-500">
                                <i class="fa-solid fa-chart-line"></i>
                            </div>
                            <h3 class="stat-label">Cukup & Kurang</h3>
                            <p class="stat-value text-yellow-500"><?= $nilaiCukup + $nilaiKurang; ?></p>
                        </div>
                    </div>
                    ...
                    <div class="card p-4 mb-6">
                        <?php if (isset($_SESSION['flash']) && $_SESSION['flash'] === "success"): ?>
                            <div class="success-message mb-4 flex items-center justify-between">
                                <span><i class="fa-solid fa-circle-check mr-2"></i> Nilai baru berhasil ditambahkan!</span>
                        ...
                        <?php if (isset($_SESSION['flash']) && $_SESSION['flash'] === "updated"): ?>
                            <div class="info-message mb-4 flex items-center justify-between">
                                <span><i class="fa-solid fa-pen-to-square mr-2"></i> Data nilai berhasil diperbarui!</span>
                        ...
                        <?php if (isset($_SESSION['flash']) && $_SESSION['flash'] === "deleted"): ?>
                            <div class="warning-message mb-4 flex items-center justify-between">
                                <span><i class="fa-solid fa-trash mr-2"></i> Data nilai berhasil dihapus!</span>
                        ...
                        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
                            <a href="tambah-nilai.php" class="btn btn-success">
                                <i class="fa-solid fa-plus mr-2"></i> Tambah Nilai
                            </a>

                            <div class="w-full lg:w-64">
                                <input type="text" id="searchInput" placeholder="Cari Nama / NIS / Mapel..."
                                    class="search-input">
                            </div>
                        </div>
                            ...
                            <div class="table-scroll">
                                <table class="w-full text-left border-collapse">
                                    ...
                                    <tbody>
                                        <?php if (empty($nilai)): ?>
                                            <tr>
                                                <td colspan="8" class="py-4 px-4 text-center text-slate-500">
                                        ...
                                        <?php else: ?>
                                            ...
                                            foreach ($nilai as $n):
                                                $nilai_kelas = '';
                                                if ($n['nilai'] >= 90) {
                                                    $nilai_kelas = 'nilai-sangat-baik';
                                                } elseif ($n['nilai'] >= 80) {
                                                    $nilai_kelas = 'nilai-baik';
                                                } elseif ($n['nilai'] >= 70) {
                                                    $nilai_kelas = 'nilai-cukup';
                                                } else {
                                                    $nilai_kelas = 'nilai-kurang';
                                                }
                                            ?>
                                                <tr class="border-t hover:bg-slate-50">
                                                    <td class="py-3 px-4"><?= $no++; ?></td>
                                                    <td class="py-3 px-4 nis"><?= htmlspecialchars($n['nis']); ?></td>
                                                    <td class="py-3 px-4 nama"><?= htmlspecialchars($n['nama_siswa']); ?></td>
                                                    <td class="py-3 px-4 kelas"><?= htmlspecialchars($n['nama_kelas']); ?></td>
                                                    <td class="py-3 px-4 mapel"><?= htmlspecialchars($n['nama_mapel']); ?></td>
                                                    <td class="py-3 px-4">
                                                        <span class="semester-badge">
                                                            <?= htmlspecialchars($n['semester']); ?>
                                                        </span>
                                                    </td>
                                                    <td class="py-3 px-4">
                                                        <span class="nilai-badge <?= $nilai_kelas ?>">
                                                            <?= htmlspecialchars($n['nilai']); ?>
                                                        </span>
                                                    </td>
                                                    <td class="py-3 px-4">
                                                        <div class="flex flex-wrap gap-2">
                                                            <a href="edit-nilai.php?id=<?= $n['id_nilai']; ?>"
                                                                class="btn btn-warning px-3 py-1 text-sm">
                                                                <i class="fa-solid fa-pen mr-1"></i> Edit
                                                            </a>
                                                            <a href="hapus-nilai.php?id=<?= $n['id_nilai']; ?>"
                                                                onclick="return confirm('Hapus nilai <?= htmlspecialchars($n['nama_siswa']) ?>?');"
                        ...
                        <?php if (empty($nilai)): ?>
                            <div class="empty-state">
                                ...
                                <p class="text-sm mt-2">Klik tombol "Tambah Nilai" untuk menambahkan data</p>
                            </div>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        searchInput.addEventListener("keyup", () => {
            ...
            rows.forEach(row => {
                ...
                if (nama.includes(filter) || nis.includes(filter) || mapel.includes(filter) || kelas.includes(filter)) {
                    row.style.display = "";
                } else {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            // Ctrl+Shift+N untuk tambah nilai
            if (e.ctrlKey && e.shiftKey && e.key === 'N') {
                e.preventDefault();
                window.location.href = 'tambah-nilai.php';
            }
        });
