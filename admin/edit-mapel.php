...
$stmt_mapel = $pdo->prepare("SELECT * FROM mata_pelajaran WHERE id_mapel = ?");
$stmt_mapel->execute([$id_mapel]);
$mapel = $stmt_mapel->fetch(PDO::FETCH_ASSOC);

// Jika mapel tidak ditemukan, redirect
if (!$mapel) {
...
// Proses form edit mata pelajaran
$error = null;
$success = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Cek apakah kode mapel sudah ada (kecuali untuk mapel ini)
        $stmt_check = $pdo->prepare("SELECT id_mapel FROM mata_pelajaran WHERE kode_mapel = ? AND id_mapel != ?");
        $stmt_check->execute([$kode_mapel, $id_mapel]);
        if ($stmt_check->fetch()) {
            throw new Exception("Kode mata pelajaran sudah terdaftar!");
        }

        // Cek apakah nama mapel sudah ada (kecuali untuk mapel ini)
        $stmt_check = $pdo->prepare("SELECT id_mapel FROM mata_pelajaran WHERE nama_mapel = ? AND id_mapel != ?");
        ...
        $stmt = $pdo->prepare("UPDATE mata_pelajaran SET nama_mapel = ?, kode_mapel = ? WHERE id_mapel = ?");
        $stmt->execute([$nama_mapel, $kode_mapel, $id_mapel]);
...
<head>
    ...
    <title>Edit Mata Pelajaran - Sistem Sekolah</title>
...
<body>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Edit Mata Pelajaran</div>
                        <div class="text-slate-500 text-xs truncate">Form edit mata pelajaran</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-book-open text-yellow-600"></i> Edit Mata Pelajaran
                        </h1>
                        <p class="text-slate-500 mt-1">Form edit mata pelajaran</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="card p-6 mb-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-lg font-semibold text-slate-700">
                                <i class="fa-solid fa-book-open mr-2"></i> Form Edit Mata Pelajaran
                            </h2>
                        ...
                        <div class="info-box mb-6">
                            <h3 class="font-semibold text-blue-800 mb-2 flex items-center">
                                <i class="fa-solid fa-info-circle mr-2"></i> Informasi Mata Pelajaran
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                <div><span class="font-medium">ID Mapel:</span> <?= htmlspecialchars($mapel['id_mapel']) ?></div>
                        ...
                        <form method="POST" class="space-y-6">
                                ...
                                <div class="space-y-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="kode_mapel" name="kode_mapel" required
                                            ...
                                            value="<?= htmlspecialchars($mapel['kode_mapel']) ?>"
                            ...
                            <div class="info-box">
                                ...
                                <ul class="text-sm text-blue-700 space-y-1">
                                    ...
                                    <li><i class="fa-solid fa-circle-small mr-2"></i> Perubahan kode akan mempengaruhi data terkait</li>
                                </ul>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-200">
                                <button type="submit" class="btn btn-success flex-1">
                                    <i class="fa-solid fa-save mr-2"></i> Update Mapel
                                </button>
                    ...
                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Edit Mata Pelajaran
                    </footer>
                </main>
            </div>
        </div>
    ...
    <script>
        ...
        function resetForm() {
            ...
            document.getElementById('nama_mapel').value = '<?= htmlspecialchars($mapel['nama_mapel']) ?>';
            document.getElementById('kode_mapel').value = '<?= htmlspecialchars($mapel['kode_mapel']) ?>';
            updatePreview();
        }
