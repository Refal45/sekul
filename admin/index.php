...
$totalSiswa = $totalGuru = $totalKelas = $totalMapel = $totalAdmin = 0;
...
try {
    ...
    $totalGuru  = $pdo->query("SELECT COUNT(*) FROM petugas WHERE role='guru'")->fetchColumn();
    ...
    $totalAdmin = $pdo->query("SELECT COUNT(*) FROM petugas WHERE role='admin'")->fetchColumn();
    ...
    // ===== Grafik Kehadiran per Bulan & Tahun =====
    $sql2 = <<<SQL
    SELECT YEAR(tanggal) as tahun, MONTH(tanggal) as bulan, COUNT(*) as total_hadir
    FROM kehadiran
    WHERE status='Hadir'
    ...
    $stmt3 = $pdo->query("SELECT jenis_kelamin, COUNT(*) as total FROM siswa GROUP BY jenis_kelamin");
    while ($row = $stmt3->fetch(PDO::FETCH_ASSOC)) {
        $labelGender[] = ($row['jenis_kelamin'] == 'L' ? 'Laki-laki' : 'Perempuan');
        $dataGender[] = $row['total'];
    }

    // ===== Grafik Doughnut Guru per Mapel =====
    $sql4 = <<<SQL
    SELECT m.nama_mapel, COUNT(DISTINCT j.id_petugas) as total_guru
    FROM jadwal j
    ...
    while ($row = $stmt4->fetch(PDO::FETCH_ASSOC)) {
        ...
        $dataGuruMapel[] = $row['total_guru'];
    }

    // ===== Data Siswa terbaru =====
    $recentSiswa = $pdo->query("SELECT nama_siswa, nis 
                                FROM siswa 
                                ORDER BY id_siswa DESC LIMIT 5")
        ->fetchAll(PDO::FETCH_ASSOC);

    // ===== Kehadiran terbaru =====
    $recentKehadiran = $pdo->query("SELECT s.nama_siswa, k.tanggal, k.status 
                                    FROM kehadiran k 
                                    JOIN siswa s ON k.id_siswa=s.id_siswa 
                                    ORDER BY k.id_kehadiran DESC LIMIT 5")
        ->fetchAll(PDO::FETCH_ASSOC);

    // ===== Nilai Tertinggi per Mapel =====
    $sqlNilaiTertinggi = <<<SQL
    SELECT m.nama_mapel, MAX(n.nilai) as nilai_max
    FROM nilai n
    JOIN mata_pelajaran m ON n.id_mapel = m.id_mapel
    GROUP BY n.id_mapel
    SQL;

    $nilaiTertinggi = $pdo->query($sqlNilaiTertinggi)
        ->fetchAll(PDO::FETCH_ASSOC);
        ...
        <aside class="sidebar flex flex-col p-4">
            ...
            <nav class="flex-1 space-y-1">
                <div class="sidebar-section">
                    ...
                    <a href="index.php" class="bg-blue-600 text-white rounded-md active">
        ...
        <div class="main-content flex-1 flex flex-col transition-all duration-300">
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-slate-500 text-xs truncate">Ringkasan data sekolah</div>
                    </div>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-chart-line text-blue-600"></i> Dashboard
                        </h1>
                        <p class="text-slate-500 mt-1">Ringkasan data sekolah & performa</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <!-- Info Cards -->
                    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6 auto-rows-fr">
                        <div class="card stat-card">
                            <div class="stat-icon text-blue-600">
                                <i class="fa-solid fa-user-graduate"></i>
                            </div>
                            <h3 class="stat-label">Total Siswa</h3>
                            <p class="stat-value text-blue-700"><?= $totalSiswa ?? 0; ?></p>
                        </div>
                        <div class="card stat-card">
                            ...
                            <p class="stat-value text-green-600"><?= $totalGuru ?? 0; ?></p>
                        </div>
                        <div class="card stat-card">
                            ...
                            <p class="stat-value text-yellow-500"><?= $totalKelas ?? 0; ?></p>
                        </div>
                        <div class="card stat-card">
                            ...
                            <p class="stat-value text-red-500"><?= $totalMapel ?? 0; ?></p>
                        </div>
                        <div class="card stat-card">
                            <div class="stat-icon text-purple-600">
                                <i class="fa-solid fa-user-shield"></i>
                            </div>
                            <h3 class="stat-label">Total Admin</h3>
                            <p class="stat-value text-purple-600"><?= $totalAdmin ?? 0; ?></p>
                        </div>
                    </div>

                    <!-- Grafik -->
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6 auto-rows-fr">
                        ...
                        <div class="card p-4 flex flex-col">
                            <h3 class="mb-3 font-semibold text-slate-700 flex items-center gap-2 text-sm lg:text-base">
                                <i class="fa-solid fa-chart-donut text-yellow-500"></i> Mapel Jadwal
                            </h3>
                            <div class="chart-container">
                                ...
                                <table class="w-full text-left border-collapse min-w-[300px]">
                                    <thead>
                                        <tr class="bg-slate-50">
                                            <th class="py-2 px-3 text-slate-600 text-xs lg:text-sm">Nama</th>
                                            <th class="py-2 px-3 text-slate-600 text-xs lg:text-sm">NIS</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php if (!empty($recentSiswa)): ?>
                                            <?php foreach ($recentSiswa as $s): ?>
                                                <tr class="border-t hover:bg-slate-50">
                                                    <td class="py-2 px-3 text-xs lg:text-sm"><?= htmlspecialchars($s['nama_siswa']); ?></td>
                                                    <td class="py-2 px-3 text-xs lg:text-sm"><?= htmlspecialchars($s['nis']); ?></td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <tr>
                            ...
                            <div class="table-scroll">
                                <table class="w-full text-left border-collapse min-w-[300px]">
                                    <thead>
                                        <tr class="bg-slate-50">
                                            <th class="py-2 px-3 text-slate-600 text-xs lg:text-sm">Nama</th>
                                            <th class="py-2 px-3 text-slate-600 text-xs lg:text-sm">Tanggal</th>
                                            <th class="py-2 px-3 text-slate-600 text-xs lg:text-sm">Status</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php if (!empty($recentKehadiran)): ?>
                                            <?php foreach ($recentKehadiran as $k): ?>
                                                <tr class="border-t hover:bg-slate-50">
                                                    <td class="py-2 px-3 text-xs lg:text-sm"><?= htmlspecialchars($k['nama_siswa']); ?></td>
                                                    <td class="py-2 px-3 text-xs lg:text-sm"><?= htmlspecialchars($k['tanggal']); ?></td>
                    ...
                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Dashboard Admin
                    </footer>
                </main>
            </div>
        </div>
