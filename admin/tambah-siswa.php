...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Validasi input
        if (empty($nama_siswa) || empty($nis) || empty($username_siswa) || empty($password)) {
            throw new Exception("Semua field wajib diisi!");
        }

        // Cek apakah NIS sudah ada
        $stmt_check = $pdo->prepare("SELECT id_siswa FROM siswa WHERE nis = ?");
        ...
        $stmt_check = $pdo->prepare("SELECT id_siswa FROM siswa WHERE username = ?");
        $stmt_check->execute([$username_siswa]);
        if ($stmt_check->fetch()) {
        ...
        // Insert data siswa
        $stmt = $pdo->prepare("INSERT INTO siswa (nama_siswa, nis, username, password, no_hp, jenis_kelamin, id_kelas) 
                              VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$nama_siswa, $nis, $username_siswa, $hashed_password, $no_hp, $jenis_kelamin, $id_kelas]);
...
<head>
    ...
    <title>Tambah Siswa - Sistem Sekolah</title>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Tambah Siswa</div>
                        <div class="text-slate-500 text-xs truncate">Form tambah data siswa baru</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-user-plus text-green-600"></i> Tambah Siswa
                        </h1>
                        <p class="text-slate-500 mt-1">Form tambah data siswa baru</p>
                    </div>
            ...
            <div class="content-wrapper content-spacing">
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="card p-6 mb-6">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-lg font-semibold text-slate-700">
                                <i class="fa-solid fa-user-graduate mr-2"></i> Form Tambah Siswa
                            </h2>
                    ...
                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Tambah Siswa
                    </footer>
                </main>
            </div>
        </div>
