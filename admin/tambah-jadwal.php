...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt_konflik->execute([$id_kelas, $hari, $jam_mulai, $jam_mulai, $jam_selesai, $jam_selesai, $jam_mulai, $jam_selesai]);
        ...
        // Insert data
        $sql = "INSERT INTO jadwal (id_kelas, id_mapel, id_petugas, hari, jam_mulai, jam_selesai) 
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id_kelas, $id_mapel, $id_petugas, $hari, $jam_mulai, $jam_selesai]);
...
<head>
    ...
    <title>Tambah Jadwal - Sistem Sekolah</title>
            ...
            <div class="mobile-header-container lg:hidden" id="mobileHeaderContainer">
                ...
                <header class="main-header mobile-header flex justify-between items-center">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm font-bold truncate">Tambah Jadwal</div>
                        <div class="text-slate-500 text-xs truncate">Form tambah jadwal pelajaran baru</div>
                    </div>
                </header>
                ...
                <header class="main-header desktop-header flex justify-between items-center">
                    <div>
                        <h1 class="text-xl font-bold flex items-center gap-2">
                            <i class="fa-solid fa-calendar-plus text-blue-600"></i> Tambah Jadwal
                        </h1>
                        <p class="text-slate-500 mt-1">Form tambah jadwal pelajaran baru</p>
                    </div>
                ...
                <main class="page-content overflow-y-auto">
                    ...
                    <div class="card">
                        <div class="form-header">
                            ...
                            <p class="text-blue-100 mt-1">Isi semua field dengan data yang valid</p>
                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="id_mapel" id="id_mapel" class="form-select" required>
                                            ...
                                            <?php foreach ($mapel as $m): ?>
                                                <option value="<?= $m['id_mapel']; ?>" <?= isset($_POST['id_mapel']) && $_POST['id_mapel'] == $m['id_mapel'] ? 'selected' : ''; ?>>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="id_petugas" id="id_petugas" class="form-select" required>
                                            ...
                                            <?php foreach ($guru as $g): ?>
                                                <option value="<?= $g['id_petugas']; ?>" <?= isset($_POST['id_petugas']) && $_POST['id_petugas'] == $g['id_petugas'] ? 'selected' : ''; ?>>
                    ...
                    <footer class="mt-6 text-center text-sm text-slate-500 select-none pb-4">
                        &copy; <?= date('Y'); ?> Sekolah - Tambah Jadwal Management
                    </footer>
                </main>
            </div>
        </div>
